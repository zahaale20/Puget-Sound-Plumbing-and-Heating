		...
		<header className="fixed top-0 left-0 z-50 flex flex-col w-full shadow-lg justify-center items-center">
			...
			<section className="flex w-full bg-white items-center justify-center">
				<div className="flex flex-row w-full max-w-7xl px-6 py-2 lg:py-0 mx-auto justify-between items-center">
					...
					>
						<ImageWithLoader
							src={getImageUrl("logo/pspah-logo.webp")}
							...
							<ul className="flex items-center flex-1">
								{ServiceLinks.map((link) => (
									...
									>
										<a
											href={link.href}
											className={`flex items-center gap-1 px-3 py-2 text-sm font-semibold text-[#0C2D70] uppercase whitespace-nowrap transition-all duration-200 border-b-4 ${openMenu === link.name ? "border-[#B32020] bg-[#F5F5F5]" : "border-transparent hover:border-[#B32020] hover:bg-[#F5F5F5]"}`}
										>
											{link.name}
											{link.submenu && (
												<FaChevronDown
										...
										{link.submenu && openMenu === link.name && (
											<div
												className="absolute left-0 top-[calc(100%+0px)] bg-white border border-gray-400 shadow-lg z-10 min-w-[250px]"
												onMouseEnter={() => handleMouseEnter(link.name)}
												onMouseLeave={() => handleMouseLeave(link.name)}
											>
												<ul className="py-2">
													{link.submenu.map((item) => (
														<li key={item.name}>
															<a
																href={item.href}
																className="block text-xs font-semibold text-[#2B2B2B] uppercase transition-all hover:bg-[#F5F5F5] px-4 py-2 whitespace-nowrap"
															>
																{item.name}
															</a>
														</li>
