...
import { TopLinks, BottomLinks } from "./navLinks3";
...
export default function Header() {
	...
	return (
		<header className="fixed top-0 left-0 z-50 w-full bg-white border-b border-gray-400 shadow-md">
			<div className="flex flex-col justify-center items-center">
				{/* Announcement Bar */}
				<div className="flex items-center justify-center w-full py-1.5 bg-[#0C2D70] shadow-sm">
				...
				{/* Middle Bar */}
				<div className="flex justify-between items-center w-full max-w-7xl h-[65px] md:h-[70px] lg:h-[80px] px-6 mx-auto">
					{/* Logo */}
							...
							<ul className="flex flex-nowrap w-full justify-end gap-6 pr-3">
								{TopLinks.map((link) => (
									<li key={link.name} className="relative group">
										<a href={link.href} className="flex items-center my-2 gap-1 text-[#0C2D70] whitespace-nowrap hover:underline cursor-pointer">
											{link.name}
										</a>
									</li>
								))}
							...
							<ul className="flex items-center flex-1">
								{BottomLinks.map((link) => (
									<li key={link.name} className="relative" onMouseEnter={() => handleMouseEnter(link.name)} onMouseLeave={() => handleMouseLeave(link.name)}>
										<a href={link.href} className={`flex items-center gap-1 px-3 py-2 text-sm font-semibold text-[#0C2D70] uppercase whitespace-nowrap transition-all duration-200 border-b-3 ${openMenu === link.name ? "border-[#B32020] bg-[#F5F5F5]" : "border-transparent hover:border-[#B32020] hover:bg-[#F5F5F5]"}`}>
											{link.name}
											{link.submenu && (
												<FaChevronDown
				...
				<div className="hidden lg:flex justify-between items-center w-full max-w-7xl px-6 h-[50px] border-t border-[#0C2D70] font-semibold">
					{/* Location */}
					<a
					...
					<div className="flex flex-row gap-2 items-center">
						<button className="flex items-center gap-2 px-3 py-2 text-white text-sm font-semibold uppercase cursor-pointer transition-all duration-300 transform whitespace-nowrap bg-[#B32020] hover:bg-[#7a1515]">
							<FaRegCalendarAlt />
							Schedule Online
						</button>
						<a
			...
			{/* Mobile Dropdown */}
			{mobileOpen && (
				<div className="absolute top-[101px] sm:top-[106px] left-0 w-full bg-white shadow-lg z-40 px-6 max-h-[calc(100vh-106px)] sm:max-h-[calc(100vh-116px)] overflow-y-auto">
